...
export default function Summary({ cards, loading, onPost, onPostThread, xEnabled }) {
  const [texts, setTexts] = useState(cards);
  const [statuses, setStatuses] = useState(cards.map(() => ({ type: "idle" })));
  ...
  function setStatus(index, status) {
    setStatuses((prev) => prev.map((cardStatus, cardIndex) => (cardIndex === index ? status : cardStatus)));
  }

  async function handlePost(text, index) {
    if (!onPost) return;
    ...
    try {
      ...
      setStatus(index, { type: "error", message: String(error) });
  ...
  if (loading) {
    return (
      <div style={styles.grid}>
        {[0, 1, 2].map((index) => (
          <div key={index} style={{ ...styles.card, ...styles.skeleton }}>
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
      <div style={styles.grid}>
        {texts.map((text, index) => {
          ...
          const status = statuses[index] ?? { type: "idle" };
      ...
      {xEnabled && onPostThread && texts.length > 1 && (
        <div style={{ display: "flex", justifyContent: "flex-end" }}>
          <button
            ...
            disabled={threadPosting || texts.some((text) => text.length === 0 || text.length > X_LIMIT)}
